---
name: funnel-stats
description: Deterministic ~~crm (HubSpot) funnel-metrics query runner — returns the month-to-date MEL/MQL/SAL/SQL/pipeline block. Delegate to it when another agent or the user needs the funnel numbers without the full revops-watchdog Tick.
model: sonnet
tools: Read, ToolSearch, Bash(python3 *)
---
