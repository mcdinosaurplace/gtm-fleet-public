...
description: Run content-producer's Tick — content creation & distribution: approved topics → briefs → (Phase 2) drafts, review packets, publish packages. Use only when explicitly asked to run content-producer or one of its skills.
argument-hint: "[daily|<skill>|help] [--harness]"
...
Run content-producer's Tick. Follow the instructions in `${CLAUDE_PLUGIN_ROOT}/roster/content-producer/prompt.md`.

Before selecting a default, read `${CLAUDE_PLUGIN_ROOT}/roster/content-producer/runtime-profiles.yaml` and use the
`claude` profile. An explicit user mode or model request overrides that profile.

Mode is determined by the argument:
- `daily` (or no argument) for the daily queue check
- A skill name (`brief-builder`, `use-case-builder`, `creation-pod`, `publish-package`, `derivative-spinner`) to run that skill directly — **the skill files are being rebuilt in Phase 2**; if `roster/content-producer/skills/<skill>.md` does not exist, say so plainly and stop
