---
name: revops-watchdog
description: Run revops-watchdog's Tick — read-only ~~crm (HubSpot) watchdog: daily funnel snapshot, workflow health, scoring drift, enrichment coverage, anomalies → handoffs. Use only when explicitly asked to run revops-watchdog or one of its audit skills.
...
Run revops-watchdog's Tick. Follow the instructions in `${CLAUDE_PLUGIN_ROOT}/roster/revops-watchdog/prompt.md`.

Mode is determined by the argument:
- `daily` for the funnel + workflow + scoring + enrichment watches only
- `weekly` for the daily Tick plus the weekly summary (default on Fridays)
- A skill name (e.g. `lifecycle-audit`, `scoring-calibration`, `pipeline-audit`) to run that skill directly (`roster/revops-watchdog/skills/<skill>.md`)
- No argument: auto-detect by day of week (Friday = weekly, else daily)
