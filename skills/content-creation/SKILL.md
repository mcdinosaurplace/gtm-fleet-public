...
## {{COMPANY}} Content Configuration

This skill is configured for **{{COMPANY}}**. Before drafting any content, read:
- `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/brand.md` — voice pillars, persona lenses, tone scale, channel matrix, competitor rules, and tone transformation examples
- `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/company.md` — company identity, strategic posture, what {{COMPANY}} does and does not compete on

### Quick Reference: {{COMPANY}} Content Rules

**Identify the persona first.** Is this content for Aaron (platform engineer, 50–500 engineers), Erin (technical founder, pre-seed → Series A) or Hannah (VP Engineering, Series B–C)? Every content decision — tone, depth, channel, humor level — flows from this.

**Lead with removal, not capability.** {{COMPANY}} messaging emphasizes what *disappears* when you use {{COMPANY}}, not what features are gained. This applies to headlines, intros, CTAs, and subject lines.

**Education leads.** Teach the practice before selling the product: define the discipline, name the patterns, give people the vocabulary ("ownership as code", "signal-to-page ratio", "the 3 a.m. tax"). Assume intelligence, not prior knowledge. Every piece should leave the reader able to *do* something — a working `.tf` file, a routing rule, an alert-hygiene checklist, a mental model.

**Close loops.** Content should resolve uncertainty, never surface new questions or transfer complexity to the reader.

**Channel tone by persona**:

| Channel | Aaron (Platform Engineer) | Erin (Technical Founder) | Hannah (VP Engineering) |
|---------|---------------------------|--------------------------|-------------------------|
| Blog / articles | A–B | B (lightly playful) | A–B |
| Social / LinkedIn | B (opinionated, never snarky) | C (conversationally funny) | B |
| Guides / resources | A–B | A–B | A |
| Product / help docs | A | A | A |
| Email | A–B | B | A |

**What to avoid in {{COMPANY}} content**:
- Fear-based reliability framing ("your next outage will cost you…", outage-shaming, downtime-cost math)
- Any claim that {{COMPANY}} prevents incidents — including "never miss an alert again"
- "Easy-to-use", "all-in-one", "AI will triage your incidents for you"
- Feature checklists or competitor comparisons by name
- Corporate announcement language ("We are thrilled to announce…")
- Naming competitors: {{COMPETITOR_A}}, {{COMPETITOR_C}}, {{COMPETITOR_D}}, {{COMPETITOR_G}}, {{COMPETITOR_B}}, {{COMPETITOR_F}}, {{COMPETITOR_H}}, {{COMPETITOR_E}}, {{COMPETITOR_I}}, {{COMPETITOR_M}}

**Factual claims**: Use {{COMPANY}}'s source hierarchy — {{COMPANY}} docs and status page → {{COMPANY}}'s own aggregate product telemetry (cited as an internal benchmark, never as a customer's data) → published citable research (DORA, the Google SRE books, vendor-neutral surveys) → reputable media and approved case studies. Aggregate benchmarks say "across {{COMPANY}} customers" and never imply a single account; named customer outcomes need a signed approval on file.

**Availability-claim rule**: uptime, delivery, and latency figures come from the status page and the current SLA — never from a deck. If a piece states an availability number, it cites the SLA it came from.
