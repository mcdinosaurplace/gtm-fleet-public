...
description: Run content-researcher's weekly Tick — voice-of-market research: mine sales calls, communities, and competitor content into an evidence-backed topic backlog. Use only when explicitly asked to run content-researcher or one of its research skills.
argument-hint: "[weekly|<skill>|help] [--harness]"
...
Run content-researcher's Tick. Follow the instructions in `${CLAUDE_PLUGIN_ROOT}/roster/content-researcher/prompt.md`.

Mode is determined by the argument:
- `weekly` (or no argument) for the weekly research sweep + topic backlog synthesis
- A skill name (e.g. `call-miner`, `topic-synthesizer`, `social-listening`, `competitor-content-scan`, `content-inventory`) to run that skill directly — if the skill file does not exist (`performance-learning` is planned), say so and stop
