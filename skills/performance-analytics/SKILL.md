...
## {{COMPANY}} Analytics Configuration

This skill is configured for **{{COMPANY}}'s Marketing & Revenue Operations** team. Read `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/ops.md` for the full team context, tech stack, funnel architecture, current priorities, and known challenges before producing any report or analysis.

### Quick Reference: {{COMPANY}}'s KPI Hierarchy

Prioritize these metrics above generic channel benchmarks:

| KPI | What It Measures |
|-----|-----------------|
| Qualified pipeline volume | By source, segment, and attribution — the north star |
| SAL-to-SQL conversion rate | Lead quality and marketing-sales alignment |
| MQL velocity | Funnel pacing vs. monthly targets |
| Data hygiene score | Property completeness, duplicate rate, validation pass rate |
| Lead handoff time | Process efficiency between Marketing and Sales |

### Tech Stack Context

- **System of record**: `~~crm` (HubSpot) — pipeline, lifecycle, attribution, lead scoring
- **BI / dashboarding**: `~~BI` (Metabase)
- **Web analytics**: `~~web analytics` (GA4)
- **Enrichment / prospecting**: `~~enrichment`, feeding {{ENRICHMENT_VENDOR}} (internal)
- **Automation**: `~~automation`

When building reports, default to `~~crm` + `~~BI` as the data source. `~~web analytics` for web/content metrics. Flag if data hygiene issues may affect reliability.

### Funnel Architecture

Prospect → Marketing Engaged → MQL → SAL → SQL → Sales Accepted Opportunity → Customer

The deal pipeline (Discovery Requested → … → Closed won / Closed lost) sits **inside** the Sales Accepted Opportunity stage — a contact lifecycle stage and a deal stage are two different layers, and only the contact layer is the funnel. `Nurture` and `Unqualified` are detours off the forward path, reachable from several stages. Canonical definitions, entrance/exit criteria, and the deal-stage crosswalk: `docs/lead-lifecycle-model.md` §4 and Appendix A.

Note: `Subscriber`, `Lead`, and `Product Qualified Lead` still exist as HubSpot `lifecyclestage` values but are not part of the canonical funnel (§6), and the legacy deck names ("Value Identified", "Demo & Business Case", "Proposal", "Quote & Signing") are not live stages — map them to deal stages before using them in any analysis. Flag anomalies in lifecycle data that reflect records parked in those states.

### Attribution Approach

{{COMPANY}} uses multi-touch attribution via HubSpot. When building attribution analysis: report by source, segment, and channel. Use first-touch to understand awareness effectiveness; use last-touch to understand conversion triggers. Flag the limitations of current attribution gaps (no product usage data in CRM yet).

### Report Format Preferences

Scott's preference: structured sections with headings and bullet checklists. Concise for quick answers; detailed for planning, scoping, or architecture. Avoid vanity metrics — lead with pipeline impact and funnel efficiency.
