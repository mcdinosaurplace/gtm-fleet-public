...
This skill is configured for **{{COMPANY}}** — an incident management and on-call platform for engineering teams. Read `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/brand.md` at the start of any brand voice task for the complete {{COMPANY}} brand reference: voice pillars, persona lenses, tone scale, channel matrix, and competitor terminology rules.

### Quick Reference: What Makes {{COMPANY}}'s Voice Distinct

**Three voice pillars**: Clear (simple, direct, reader-first) → Authoritative (expert, not arrogant) → Approachable (warm colleague, not corporate)

**Canonical truth**: {{COMPANY}} works because the pages stop arriving. Engineers don't babysit alerts — they keep shipping.

**Core messaging principle — removal over capability**: Lead with what *disappears*, not what's added. The win is the page that never fires, the escalation that never needed a human router, the Sunday that stayed a Sunday. The canonical measure: "Does this close a loop or open one?"

**The cardinal sin**: Fear-based reliability. Never lead with outage-shaming, downtime-cost math, or worst-case scenarios. The reader is already worried about reliability — that is why they are reading. Our job is to make them more capable, not more afraid.

**The claim that is never made**: {{COMPANY}} does not prevent incidents. It shortens them, routes them, and helps teams learn from them. "Never miss an alert again" is a hard fail — it names the opposite of the reader's actual problem.

**Three personas to calibrate for**:
- **Aaron the Platform Engineer** (primary ICP; SRE / platform lead, 50–500 engineers) — educational confidence, technical precision without gatekeeping, humor level A–B
- **Erin the Technical Founder** (founder or CTO, pre-seed → Series A, 5–40 engineers) — energetic clarity, conversational and pragmatic, humor level B
- **Hannah the VP Engineering** (VP / Head of Engineering, Series B–C, 90–200 engineers) — reassuring authority, precise and evidence-led, humor level A only

**Humor scale**: A = dry/understated, B = lightly playful, C = conversationally funny (social only). Never sarcasm, snark toward competitors, or jokes at the expense of an engineer's bad night.

**Competitor terms**: Never name {{COMPETITOR_A}}, {{COMPETITOR_C}}, {{COMPETITOR_D}}, {{COMPETITOR_G}}, {{COMPETITOR_B}}, {{COMPETITOR_F}}, {{COMPETITOR_H}}, {{COMPETITOR_E}}, {{COMPETITOR_I}}, or {{COMPETITOR_M}}. Use "traditional platforms" or "legacy tools" instead.

**Before and after tone transformations** (and more examples) are in `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/brand.md`.
