...
This skill is configured for **{{COMPANY}}**. Before planning any campaign, read `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/company.md` for {{COMPANY}}'s growth model, customer segments, strategic posture, and non-negotiable guardrails. Also reference `${CLAUDE_PLUGIN_ROOT}/context/brand-pack/brand.md` for persona-channel tone guidance.

### Quick Reference: {{COMPANY}} Campaign Rules

**All-bound model**: Acquisition, activation, retention, and expansion are a continuous system — not isolated motions. Campaigns should be built with this in mind; even awareness campaigns should consider downstream activation and retention.

**Audience priority**:
1. **Aaron the Platform Engineer** — mid-market B2B SaaS engineering orgs, 50–500 engineers; owns the paging stack, evaluates through the API and Terraform provider docs, runs the trial and writes the internal recommendation (gold medal segment)
2. **Erin the Technical Founder** — U.S.-based founders and CTOs, pre-seed → Series A, 5–40 engineers, distributed team; self-serve entry, sole decision-maker
3. **Hannah the VP Engineering** — Series B/C growth teams, 90–200 engineers; Platform / Security / Finance buying committee

**Strategic posture**: {{COMPANY}} is engineer-first, not manager-first. Orient campaigns toward practical complexity, mid-market software companies that run their own production systems, and engineer-led or engineer-influenced buying — not enterprise ITSM suites or feature checklists. Sales assistance starts above roughly 75 responders; below that the motion is product-led.

**Channel guidance by persona**:

| Persona | High-Value Channels |
|---------|-------------------|
| Aaron | r/sre and r/devops, Hacker News, GitHub, CNCF and platform-engineering Slack channels, SREcon, KubeCon platform tracks, DevOpsDays, provider docs and changelog feeds |
| Erin | Hacker News, Product Hunt, Indie Hackers, founder Slack communities, VC/advisor referrals, developer newsletters |
| Hannah | LinkedIn, the Rands Leadership Slack, engineering-leadership newsletters, G2 / Capterra, QCon, SREcon leadership tracks, peer roundtables |

**Campaign brief template** ({{COMPANY}} standard): Overview → Objective → Audience → Core Benefits → Messaging → Competitive Landscape → Goals & KPIs → Channel Plan → Notes

**Success metrics to prioritize**: Qualified pipeline by source and segment, SAL-to-SQL conversion rate, MQL velocity. Avoid vanity metrics.

**Messaging guardrails for campaigns**:
- Lead with removal of friction, not feature announcements
- Do not use fear-based reliability angles — no outage-shaming, no downtime-cost math
- Never claim or imply that {{COMPANY}} prevents incidents; it shortens them, routes them, and helps teams learn from them
- Do not position against named competitors ({{COMPETITOR_A}}, {{COMPETITOR_C}}, {{COMPETITOR_D}}, {{COMPETITOR_G}}, {{COMPETITOR_B}}, {{COMPETITOR_F}}, {{COMPETITOR_H}}, {{COMPETITOR_E}}, {{COMPETITOR_I}}, {{COMPETITOR_M}})
