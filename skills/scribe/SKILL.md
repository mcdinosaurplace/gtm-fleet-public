---
name: scribe
description: Run Scribe — weekly business review (WBR) draft from revops-watchdog/performance-marketer state, Monday stand-up, Tue–Fri Linear pulse, Notion topic sync. Use only when explicitly asked to run Scribe.
argument-hint: "[weekly|standup|pulse|notion-topic-sync|wbr-prep|help] [--notify=dm|channel] [--dry-run] [--harness]"
...
Run Scribe. Follow the instructions in `${CLAUDE_PLUGIN_ROOT}/roster/scribe/prompt.md`.

Mode is determined by the argument:
- `weekly` (default, Wednesdays) — the full WBR prep flow; posts to the team channel
- `wbr-prep` — alias for `weekly` with channel notification forced
- `standup` — Monday async stand-up post (`standup sweep` = Tuesday EOD capture)
- `pulse` — Tue–Fri read-only ~~issue tracker (Linear) health pulse
- `notion-topic-sync` — only the ~~knowledge base topic-backlog sync (`roster/scribe/skills/notion-topic-sync.md`), skipping the mode work; tickable headless as `--agent scribe --mode notion-topic-sync`
- `--notify=dm` or `--notify=channel` — notification routing for ad-hoc runs; `--channel=<name>` overrides the default channel
- `--dry-run` — save the local preview to `wbr/YYYY-MM-DD.md`, skip ~~knowledge base + ~~chat writes
